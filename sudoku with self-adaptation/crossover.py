...
def uniform_crossover(pop, cx_rate):
    ...
    for i in range(len(pop_copy)):
        ...
        for j in range(int(len(child)/2)):
            rand = random.random()
            if rand < cx_rate:
                index = random.randint(0, len(pop_copy)-1)
                child[j] = pop_copy[index][j]
        for j in range(int(len(pop_copy[i])/2), int(len(pop_copy[i]))):
            rand = random.random()
            if rand < cx_rate:
                index = random.sample(range(len(pop)), 2)
                child[j] = (pop_copy[index[0]][j] +pop_copy[index[1]][j])/2
...
def order_crossover(pop, pos):
    ...
    for num in range(int(len(pop)/2)):
        ...
        parent1, parent2 = pop_copy[0][:9], pop_copy[1][:9]
        mut_rates1, mut_rates2 = pop_copy[0][9:], pop_copy[1][9:]
        child1 = []
        child2 = []
        for i in range(int(len(parent1))):
        ...
        children.append(child1+mut_rates1)
        children.append(child2+mut_rates2)
    return children
